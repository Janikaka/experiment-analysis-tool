"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var router_1 = require('@angular/router');
var experiment_server_service_1 = require('./experiment-server.service');
var ExperimentsComponent = (function () {
    function ExperimentsComponent(router, experimentServerService) {
        this.router = router;
        this.experimentServerService = experimentServerService;
    }
    ExperimentsComponent.prototype.getExperiments = function () {
        var _this = this;
        this.experimentServerService.getExperiments().then(function (experiments) { return _this.experiments = experiments; });
    };
    ExperimentsComponent.prototype.ngOnInit = function () {
        this.getExperiments();
    };
    ExperimentsComponent.prototype.showMetadata = function (id) {
        this.router.navigate(['/experiments', id]);
    };
    ExperimentsComponent = __decorate([
        core_1.Component({
            selector: 'experiments',
            templateUrl: 'app/experiments.component.html',
        }), 
        __metadata('design:paramtypes', [router_1.Router, experiment_server_service_1.ExperimentServerService])
    ], ExperimentsComponent);
    return ExperimentsComponent;
}());
exports.ExperimentsComponent = ExperimentsComponent;
//# sourceMappingURL=experiments.component.js.map